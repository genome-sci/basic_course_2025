...
- **bash**や**zsh**などの様々な派生シェル（いわゆる方言。機能や文法等が異なる）が存在する  
...
- PythonやR等と違い事前に用意されているライブラリは特には無い。
  そのため、ChatGPT等で同じ処理をPythonとシェルスクリプトの両方でプログラムを書かせると、シェルスクリプトでは複雑なコードになることがある。  
- コードの可読性に難あり  
...
3. コードの解説  
`v=5` は、変数vに5を代入。変数の定義では=の前後にスペースを入れない。値の代入時には$は不要。代入する値は文字列でも数値でも良い。文字列の代入の場合は、
